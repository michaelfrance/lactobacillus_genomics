...
for rec in SeqIO.parse(open(sys.argv[1],"r"), "fasta") :
    ...
    fasta_out.write('>fragment' + str(rec.id) + '\n' + str(rec.seq))
    ...
    while fail_count < max_failures:
        try:
            blastp_cline = NcbiblastpCommandline(query="fragment.fasta", db="~/bioinformatics_tools/ncbi-blast/db/nr", outfmt=5, out="fragment.xml",num_threads=1,evalue=0.001,num_descriptions=10,num_alignments=10)
